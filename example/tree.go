...
type IndexPage struct { // index page
	c int
	x [2*INDEX_MAX + 2]Index
}
...
func (q *IndexPage) insert(i int, d *DataPage, ch interface{}) *IndexPage {
	c := q.c
	if i < c {
		q.x[c+1].ch = q.x[c].ch
...
func (t *Tree) catX(p, q, r *IndexPage, pi int) {
	t.ver++
	...
	for {
		...
		if ok {
			...
			case *IndexPage:
				dp := x.x[i].sep
				switch {
				case dp.c > RECORD_MAX:
					t.extract(dp, 0)
				default:
					if x.c < INDEX_MAX && q != t.r {
...
func (t *Tree) extract(q *DataPage, i int) { // (r []byte) {
	t.ver++
...
func (t *Tree) insert(q *DataPage, i int, k []byte, v []byte) *DataPage {
	t.ver++
	c := q.c
	if i < c {
...
func (t *Tree) overflow(p *IndexPage, q *DataPage, pi, i int, k []byte, v []byte) {
	t.ver++
	l, r := p.siblings(pi)

	if l != nil && l.c < 2*RECORD_MAX {
		l.mvL(q, 1)
		t.insert(q, i-1, k, v)
		return
	}

	if r != nil && r.c < 2*RECORD_MAX {
		if i < 2*RECORD_MAX {
			q.mvR(r, 1)
			t.insert(q, i, k, v)
		} else {
...
func (t *Tree) Set(k []byte, v []byte) {
	...
	z := t.insert(&DataPage{}, 0, k, v)
	t.r, t.first, t.last = z, z, z
	return
}

func (t *Tree) split(p *IndexPage, q *DataPage, pi, i int, k []byte, v []byte) {
	t.ver++
	r := &DataPage{}
	if q.n != nil {
		r.n = q.n
	...
	copy(r.d[:], q.d[RECORD_MAX:2*RECORD_MAX])
	for i := range q.d[RECORD_MAX:] {
		q.d[RECORD_MAX+i] = zde
	}
	q.c = RECORD_MAX
	r.c = RECORD_MAX
	if pi >= 0 {
		...
		t.r = newIndexPage(q).insert(0, r, r)
	}
	if i > RECORD_MAX {
		t.insert(r, i-RECORD_MAX, k, v)
		return
	}
...
func (t *Tree) splitX(p *IndexPage, pp **IndexPage, pi int, i *int) {
	t.ver++
	q := *pp
	r := &IndexPage{}
	copy(r.x[:], q.x[INDEX_MAX+1:])
	q.c = INDEX_MAX
	r.c = INDEX_MAX
	if pi >= 0 {
		p.insert(pi, q.x[INDEX_MAX].sep, r)
	} else {
		t.r = newIndexPage(q).insert(0, q.x[INDEX_MAX].sep, r)
	}
	q.x[INDEX_MAX].sep = nil
	for i := range q.x[INDEX_MAX+1:] {
		q.x[INDEX_MAX+i+1] = zxe
	}
	if *i > INDEX_MAX {
		*pp = r
		*i -= INDEX_MAX + 1
...
func (t *Tree) underflow(p *IndexPage, q *DataPage, pi int) {
	t.ver++
	l, r := p.siblings(pi)

	if l != nil && l.c+q.c >= 2*RECORD_MAX {
		l.mvR(q, 1)
	} else if r != nil && q.c+r.c >= 2*RECORD_MAX {
		q.mvL(r, 1)
		r.d[r.c] = zde // GC
	} else if l != nil {
		t.cat(p, l, q, pi-1)
...
func (t *Tree) underflowX(p *IndexPage, pp **IndexPage, pi int, i *int) {
	t.ver++
	var l, r *IndexPage
	q := *pp
	...
	if pi >= 0 {
		if pi > 0 {
			l = p.x[pi-1].ch.(*IndexPage)
		}
		if pi < p.c {
			r = p.x[pi+1].ch.(*IndexPage)
		}
	}

	if l != nil && l.c > INDEX_MAX {
